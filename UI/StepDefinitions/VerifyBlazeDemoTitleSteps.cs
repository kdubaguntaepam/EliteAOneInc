    ...
    public class VerifyBlazeDemoTitleSteps
    {
        ...
        public void ThenThePageTitleShouldBe(string expectedTitle)
        {
            try
            {
                ...
                Assert.That(expectedTitle.Equals(actualTitle));
            }
            catch (Exception ex)
            {
                ScreenshotHelper.TakeScreenshot(_driver, "BlazeDemoTitleVerificationFailure");
                throw new Exception($"Title verification failed: {ex.Message}");
            }
        }
    }
}